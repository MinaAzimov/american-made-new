...
export default class TextDisperseTitle extends Component {
	...
	renderText() {
		...
		for (let i = 0; i < text.length; i++) {
			...
			let c = text[i];
			if (c === ' ') {
				elements.push(<span key={i} className="text-disperse-char-blank-title">{' '}</span>);
			}
			else {
				let style = (isActive) ? {
				...
				let e = <span className="text-disperse-char-title" key={i} style={style}>{c}</span>
				elements.push(e);
			}
